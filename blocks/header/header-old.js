...
function handleNavToggle({ target }) {
    ...
    block.querySelectorAll(":scope div")?.forEach((el, index) => {
      ...
      if (nestedNav) {
        nestedNav.forEach((nav) => {
          ...
          const nestedNavToggle = document.createElement("button");
          nestedNavToggle.title = "Toggle Menu";
          nestedNavToggle.setAttribute('aria-expanded', false);
          nestedNavToggle.addEventListener('click', handleNavToggle);
          nestedNavToggle.addEventListener('focus', handleNavToggle);
          /*
          nestedNavToggle.addEventListener('mouseover', handleNavToggle);
          nestedNavToggle.addEventListener('mouseout', handleNavToggle);
          */
          nestedNavParent.classList.add("header-nav-nested");
          nav.before(nestedNavParent);
          nestedNavParent.append(nav);
          nestedNavParent.before(nestedNavToggle);
