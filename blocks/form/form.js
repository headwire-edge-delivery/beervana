...
const formCreateElementLookup = {
  "text-field": "input",
  "submit": "input",
};

const formItemTypeLookup = {
  "text-field": "text",
  "email": "email",
...
export default async function decorate(block) {
  ...
  const input = block;
  const opts = {
    path: block.querySelector('a').getAttribute('href'),
  };

  const { data } = await fetchJSON(opts);
  console.log("formJSON data", data);
  ...
  if (data?.length > 0) {
    ...
    data?.forEach((item) => {
      console.log("formJSON item", item);
      if (item.Type) {
        const formItemWrapper = document.createElement("div");
        formItemWrapper.classList.add("form-item-wrapper");
        ...
        const formItem = document.createElement(formCreateElementLookup[item.Type]);
        formItemWrapper.appendChild(formItem);

        if (item.Field) formItem.setAttribute("id", item.Field);
        if (item.Format) formItem.setAttribute("type", formItemTypeLookup[item.Format]);
        if (item.Placeholder) formItem.setAttribute("placeholder", item.Placeholder);

        formWrapper.appendChild(formItemWrapper);
      }
    });
